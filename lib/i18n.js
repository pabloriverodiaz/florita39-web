...
(function () {
  ...
  var lang = 'en';
  try {
    // ?lang=es|en (used by the /es and /en SEO redirects from the old site) wins,
    // and is remembered; otherwise fall back to the saved choice, else English.
    var qpLang = new URLSearchParams(location.search).get('lang');
    if (qpLang === 'es' || qpLang === 'en') { lang = qpLang; localStorage.setItem('f39_lang', qpLang); }
